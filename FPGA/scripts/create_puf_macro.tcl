...
for {set i 0} {$i < [expr $n_bistable*16]} {incr i} {
...
for {set i 0} {$i < [expr $n_bistable/2]} {incr i} {
	...
	lappend comps [place_puf_bistable [expr 2*$i] $n_inverters [expr $start_row+$i] $start_colum 0 $start_colum C D A B]
	lappend comps [place_puf_bistable [expr 2*$i+1] $n_inverters [expr $start_row+$i] $start_colum 2 [expr $start_colum + $n_inverters/4] B D A C]
...
for {set i 0} {$i < [expr $n_bistable/2]} {incr i} {
	...
	set bist_id_1 [expr 2*$i+1]
	set comps0 [lindex $comps $bist_id_0]
	set comps1 [lindex $comps $bist_id_1]
	route_puf_bistable $bist_id_0 [lindex $comps0 0] [lindex $comps0 1] $n_inverters [expr $start_row+$i] $start_colum 0 $start_colum C D A B
